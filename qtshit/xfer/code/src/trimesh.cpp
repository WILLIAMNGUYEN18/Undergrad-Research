...
Vec3d &TriMesh::getTriTexCoords(int face, int vert) {
	return triTexCoords[face*3 + vert];
}
...
bool TriMesh::gsTriTexCoords(int action) {
	if (action == 0) {
		hasTriTexCoords = false;
		return false;
	}
	else if (action == 1) {
		hasTriTexCoords = true;
		triTexCoords.resize(numTris() * 3);
		return true;
	}
	else {
		return hasTriTexCoords;
	}
}
...
bool TriMesh::loadFile(char *fname) {
...
bool TriMesh::saveFile(char *fname) {
	int l = (int)strlen(fname);
	...
	while (!in.eof()) {
		...
		if (s[0] == 'v' && s[1] == ' ') {
			...
			for (i=2; i < verts.size(); i++)
				addTri(verts[0], verts[i], verts[i - 1]);
		}
	}
