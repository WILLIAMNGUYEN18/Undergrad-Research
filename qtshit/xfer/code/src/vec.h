...
public:
	...
	T& operator []( int i ) { 
		if (i < 0 || i >= numElements) 
			cout << "warning: vector access out of range" << endl;
		return n[i]; 
	}
...
public:
	...
	void glColor(double alpha = 1.0)  { glColor4d(n[0], n[1], n[2], alpha); }
...
void Vec<T>::resize( int size, bool zeroElements ) {
	if( numElements != size ) {
		...
		if (n) {
			if (!zeroElements)
				memcpy(newN, n, sizeof(T) * min(size, numElements));
			delete []n;
		}
		numElements = size;
		n = newN;
...
template <class T>
inline Vec3<T> mult4m3v(const Mat4<T>& a, const Vec3<T>& v){
	return Vec3<T>( a.n[0]*v.n[0]+a.n[1]*v.n[1]+a.n[2]*v.n[2]+a.n[3],
					a.n[4]*v.n[0]+a.n[5]*v.n[1]+a.n[6]*v.n[2]+a.n[7],
					a.n[8]*v.n[0]+a.n[9]*v.n[1]+a.n[10]*v.n[2]+a.n[11]) /  
					(a.n[12]*v.n[0]+a.n[13]*v.n[1]+a.n[14]*v.n[2]+a.n[15]);
}
...
template <class T>
inline void swapVec( Vec3<T>& a, Vec3<T>& b ) {
	Vec3<T> t( a );
	a = b;
	b = t;
}
...
template <class T>
Vec3<T> vec4to3(const Vec4<T> &v) {
	return Vec3<T>(v[0], v[1], v[2]);
}

template <class T>
Vec4<T> vec3to4(const Vec3<T> &v) {
	return Vec4<T>(v[0], v[1], v[2], 1);
}

template <class T>
Vec4<T> vec3to4z(const Vec3<T> &v) {
	return Vec4<T>(v[0], v[1], v[2], 0);
}
