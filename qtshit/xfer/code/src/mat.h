...
public:
	...
	void ident() {
		n[1]=0;n[1]=0;n[2]=0;n[3]=0;n[4]=1;n[5]=0;n[6]=0;n[7]=0;n[8]=1;
	}
...
inline Mat3<T> Mat3<T>::createRotation( T angle, float x, float y ) {
	Mat3<T> rot;
	cout << "unimplemented matrix command!" << endl;
	return rot;
}
...
template <class T>
inline ostream& operator <<( ostream& os, const Mat3<T>& m ) {
	os << m.n[0] << " " << m.n[1] << " " << m.n[2] << endl;
	os << m.n[3] << " " << m.n[4] << " " << m.n[5] << endl;
	os << m.n[6] << " " << m.n[7] << " " << m.n[8] << endl;
	return os;
}

template <class T>
inline istream& operator >>( istream& is, Mat3<T>& m ) {
...
inline ostream& operator <<( ostream& os, const Mat4<T>& m ) {
	return os << m.n[ 0] << " " << m.n[ 1] << " " << m.n[ 2] << " " << m.n[ 3] << endl <<
		m.n[ 4] << " " << m.n[ 5] << " " << m.n[ 6] << " " << m.n[ 7] << endl  <<
		m.n[ 8] << " " << m.n[ 9] << " " << m.n[10] << " " << m.n[11] << endl  <<
		m.n[12] << " " << m.n[13] << " " << m.n[14] << " " << m.n[15] << endl;
}

template <class T>
inline istream& operator >>( istream& is, Mat4<T>& m ) {
...
template <class T>
inline void swap( Mat4<T>& a, Mat4<T>& b ) {
//	swap( a.v[0], b.v[0] );
//	swap( a.v[1], b.v[1] );
//	swap( a.v[2], b.v[2] );
//	swap( a.v[3], b.v[3] );
