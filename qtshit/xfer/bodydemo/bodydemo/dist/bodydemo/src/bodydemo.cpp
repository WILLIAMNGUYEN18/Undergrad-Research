...
void FeatureMat::load(char *fname, char *invFName) {
	...
	if (!openIFStream(&in, invFName, "inverse feature matrix"))
		return;
	invData = new float[numPCA*numFeatures];for (i=0; i < numFeatures; i++) {
		for (j=0; j < numPCA; j++) {
	...
	if (alt) {
		...
		if (calcRayIntersection(&curMesh, orig, direction)){
			if (hitNeg) {
				...
				if (selectedPtInd < 0) {
					selectedPtInd = pcaGF->markerIndices.size();
...
//	strcpy(setName, "data/dataset-male-ea.txt");
//	strcpy(setName, "male-ea.dat");
	...
	curFeature = 0;
	resetToAverage();
//	resetFromPly("data/boxer-trans2.ply");
//	applyDelta();
	PCAData data;
