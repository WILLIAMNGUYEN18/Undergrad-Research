...
void FastTriMesh::renderSmoothWF(bool wfOnly, float r, float g, float b) {
	if (lastDrawn != this) {
		dirtyVerts = true;
		dirtyColors = true;
		lastDrawn = this;
...
void FastTriMesh::renderWF(float r, float g, float b) {
	glDisable(GL_LIGHTING);
	...
	for (i=0; i < numTris; i++) {
		glBegin(GL_LINE_STRIP);
			for (j = 0; j < 4; j++) {
				int ofs = 3*tris[i*3 + (j%3)];
				glVertex3f(verts[ofs+0], verts[ofs+1], verts[ofs+2]);
			}
		glEnd();
	}
