...
void
quat_to_axis(float q[4], float a[3], float *phi )
{
  float tf[4];
...
void 
euler_to_quat(float ang[3],float q[4])
{
 float qt[4];
 float a[3]={ 1.0, 0.0, 0.0 };
 axis_to_quat(a,ang[0],qt);
 a[0]=0.0; a[1]=1.0; a[2]=0.0;
 axis_to_quat(a,ang[1],q);
 multiply_quats(q,qt,qt);
 a[0]=0.0; a[1]=0.0; a[2]=1.0; 
 axis_to_quat(a,ang[2],q);
 multiply_quats(q,qt,q);
}
...
void
multiply_quats(float q1[4], float q2[4], float dest[4])
{
    float t1[4], t2[4];
    float tf[4];
...
void
rotmatrix_to_euler(float m[4][4],float a[3])
{
 float *mat;
